...
static void UploadRayMesh(std::vector<GLfloat> combinedVertices){
    GLuint vbo;
    glGenBuffers(1, &vbo);
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferData(GL_ARRAY_BUFFER, combinedVertices.size() * sizeof(GLfloat), combinedVertices.data(), GL_STATIC_DRAW);
...
static DrawDetails UploadRayMesh2(
    const std::vector<GLfloat>& verts,
    const std::vector<GLfloat>& colors,
    const std::vector<GLuint>& elems)
{
    GLuint vboHandles[2];
    glGenBuffers(2, vboHandles);
    GLuint posBufferHandle = vboHandles[0];
